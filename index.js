 ...
 *
 * 15-10-2023 Support of new SSO
 *
 *
 * to revert back to old SSO just set const oldSSO to true;
 *
 *************************************************************/
...
// ********  WARNING  ******************
// Set oldSSO=true with https://sso-test.sch.gr
// Set oldSSO=false with https://sso-01-test.sch.gr
const oldSSO = false;
// **************************************
...
export default class Cas {
	constructor(options) {
		...
		if (this.cas_version === '1.0') {
			...
			_validateUri = this.cas_version === '2.0' ? '/serviceValidate' : '/p3/serviceValidate';
			_validate = function(body, callback) {
				parseXML(
					body,
					{
					...
					(err, result) => {
						...
						try {
							...
							if (success !== 'Success') {
								...
								//
								//*********************************************************
								var attributesArray = oldSSO ? atrributeStatement[0].attribute : atrributeStatement.attribute;
								...
								attributesArray.forEach(function(attr) {
									...
									if (attr.attributevalue instanceof Array) {
										...
										attr.attributevalue.forEach(function(v) {
											...
											//*********************************************************
											oldSSO ? thisAttrValue.push(v._) : thisAttrValue.push(v);
										});
									} else {
										//*********************************************************
										//  OLD CAS
										...
										//*********************************************************
										thisAttrValue = oldSSO ? attr.attributevalue._ : attr.attributevalue;
									}
